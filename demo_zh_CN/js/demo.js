...
function Events1() { // demo 
	...
	function on_page_loaded(task) { 
		...
		// });
		task.create_menu($("#menu"), $("#content"), {
			splash_screen: '<h1 class="text-center">Jam.py 演示应用程序</h1>',
			view_first: true
		});
...
function Events15() { // demo.catalogs.tracks 
	...
	function on_view_form_created(item) {
		if (!item.lookup_field) {
			...
			item.alert('双击底部表格中的记录，即可查看该曲目的销售发票。');
	...
	function set_media_type(item) {
		...
		if (selections.length > 1000) {
			...
			copy.edit_options.title = '将媒体类型设置为 ' + selections.length + ' 记录';
					...
					try {
						...
						item.server('set_media_type', [c.media_type.value, selections], function(res, error) {
							...
							if (res) {
								...
								c.cancel_edit();
								item.alert(selections.length + ' 记录已被修改。');
							}
						});
					}
					finally {
...
function Events16() { // demo.journals.invoices 
	...
	function on_view_form_created(item) {
		...
		set_paid_btn(item);
		// item.server('get_sales', function(res, error) {
		//					 if (error) {
		//						 item.alert_error(error);
		//					 }
		//					 if (res) {
	...
	function show_cusomers(item, ctx) {
		...
		inv.open(
			...
			function() {
				...
				inv.first();
				draw_chart(item, ctx, labels, data, colors, '十大最活跃客户');
	...
	function show_tracks(item, ctx) {
		...
		tracks.open(
			...
			function() {
				...
				draw_chart(item, ctx, labels, data, colors, '十大最受欢迎歌曲');
...
function Events60() { // demo.journals.inv_get_sales 
	...
	function process_table_row(row, item) {
		// Root rows = customers
		if (row.level === 0) {
			row.css('background-color', '#d3d3d3'); // light gray
		...
		if (row.level === 1) {
			...
			if (row.total.value > 50) {
				row.css('color', 'green');
			} else if (row.total.value < 40) {
				row.css('color', 'red');
			}
		}
	}
	...
	this.process_table_row = process_table_row;
}

task.events.events60 = new Events60();

function Events62() { // demo.analytics.ag 

	function on_view_form_created(item) {
		 show_sales_grid_3(item);
	}
	// function on_view_form_created(item) {
	...
	//	 var rowData = [
	//		 { id: 1, name: 'Almeida', total: 41.60, level: 0, expanded: false, visible: true },
	//		 { id: 2, name: 'INV-12', total: 20.10, level: 1, parent: 1, visible: false },
	//		 { id: 3, name: 'INV-34', total: 21.50, level: 1, parent: 1, visible: false },
	
	//		 { id: 4, name: 'Barnett', total: 45.82, level: 0, expanded: false, visible: true },
	//		 { id: 5, name: 'INV-28', total: 45.82, level: 1, parent: 4, visible: false }
	//	 ];
	...
	//								 renderGrid(); // re-render grid safely
	//							 };
	//						 }
	//						 return div;
	//					 }
	...
	function show_sales_grid_3(item) {
	
		item.server('get_sales_3', function(rowData) {
			...
			function renderGrid() {
				...
				// 🔴 VERY IMPORTANT: destroy previous grid
				if (gridDiv.__agGridInstance) {
					gridDiv.__agGridInstance.destroy();
				}
					...
					columnDefs: [
						{
							...
							cellRenderer: function(params) {
								...
								var div = document.createElement('div');
								div.style.paddingLeft = indent + 'px';
								div.style.cursor = level < 2 ? 'pointer' : 'default';
								div.innerHTML = icon + params.value;
								...
								if (level < 2) {
									...
									div.onclick = function() {
											...
											if (params.data.expanded) {
												...
												hideDescendants(rowData, params.data.id);
	...
	function hideDescendants(rowData, parentId) {
		rowData.forEach(function(r) {
			if (r.parent === parentId) {
				r.visible = false;
				r.expanded = false;
				hideDescendants(rowData, r.id); // recurse
			}
		});
	}
	this.on_view_form_created = on_view_form_created;
	this.show_sales_grid = show_sales_grid;
	this.show_sales_grid_3 = show_sales_grid_3;
